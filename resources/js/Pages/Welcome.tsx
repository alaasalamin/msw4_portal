...
function LoginWidget({ canResetPassword }) {
    const [tab, setTab] = useState('customer');
    const [showPw, setShowPw] = useState(false);

    const customerForm = useForm({ email: '', password: '', remember: false });
    const partnerForm  = useForm({ email: '', password: '', remember: false });
...
export default function Welcome({ auth, canLogin, canRegister, canResetPassword }) {
            ...
            <section id="services" className="bg-slate-50 py-20 sm:py-28">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="mx-auto max-w-2xl text-center">
                        ...
                        <h2 className="font-display mt-3 text-3xl font-normal text-slate-900 sm:text-4xl">
                            Every device, every problem
                        </h2>
            ...
            <section className="bg-slate-50 py-16 sm:py-24">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="grid gap-6 sm:grid-cols-3">
                        {[
                            {
                                quote: 'Moonrepair turned our device repair backlog from weeks into days. The partner portal is genuinely excellent.',
                                name: 'Sarah M.',
                                role: 'Operations Lead, TelecomPlus',
                            },
                            {
                                quote: 'Dropped my phone in the morning, had a diagnosis by noon and it was ready by closing time. Incredible.',
                                name: 'Ahmed K.',
                                role: 'Individual Customer',
                            },
                            {
                                quote: 'The real-time tracking and consolidated invoicing save us hours every month. Highly recommended.',
                                name: 'Lisa T.',
                                role: 'Procurement Manager, RetailChain AG',
                            },
                        ].map(({ quote, name, role }) => (
